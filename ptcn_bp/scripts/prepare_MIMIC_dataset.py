...
def CreateWindows(win_len, fs, N_samp, overlap):
    ...
    N_samp = N_samp - win_len + 1 # 窗口起点最大只能到当前值
    ...
    for idx, dirs in enumerate(SubjectDirs):
        ...
        for file in DataFiles:
            ...
            for i in range(0, N_win):
                ...
                NN = np.diff(sys_p_win)/fs # np.diff()计算相邻元素的差。
                HR = 60/np.mean(NN)
                if HR < 50 or HR > 140:
                    output[i, :] = np.nan
                    continue
                ...
                BP_sys = np.median(ABP_sys_win).astype(int) # 取中位数
        ...
        # save data is at least 100 good samples have been extracted #最少100条数据写入一次输出文件，要不然写太多次不好
